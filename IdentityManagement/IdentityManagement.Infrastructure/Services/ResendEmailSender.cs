...
namespace IdentityManagement.Infrastructure.Services
{
    public sealed class ResendEmailSender(ResendClient resend) : IEmailSender
    {
        ...
        public async Task SendSignupVerificationEmailAsync(string toEmail, string companyName, string confirmLink, int expiresInHours, CancellationToken cancellationToken = default)
        {
            var message = new EmailMessage();
            message.From = "no-reply@mainstay.com.br";
            message.To.Add(toEmail);
            message.Subject = "Confirme seu e-mail — Mainstay";
            message.HtmlBody = BuildSignupVerificationHtml(companyName, confirmLink, expiresInHours);
                          ...
                          <table cellpadding="0" cellspacing="0">
                            <tr>
                              <td style="border-radius:6px;background:#1F3B61;">
                                <a href="{confirmLink}" style="display:inline-block;padding:14px 32px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;border-radius:6px;">Confirmar e criar minha conta</a>
                              </td>
                            </tr>
                          </table>
