...
export class ContractService {
  ...
  static async getAll(params?: PaginationParams): Promise<PaginatedResult<Contract>> {
    const response = await httpClient.get<Contract[]>(this.baseUrl)
  ...
  static async getByCompanyId(companyId: number): Promise<Contract[]> {
    const response = await httpClient.get<Contract[]>(`${this.baseUrl}/company/${companyId}`)
    return response.data ?? []
  }

  static async getBySystemApplicationId(systemApplicationId: number): Promise<Contract[]> {
    const response = await httpClient.get<Contract[]>(`${this.baseUrl}/system-application/${systemApplicationId}`)
    return response.data ?? []
  }

  static async create(contract: CreateContractRequest): Promise<Contract> {
    const response = await httpClient.post<Contract>(this.baseUrl, {
      ...contract,
  ...
  static async update(id: number, contract: UpdateContractRequest): Promise<Contract> {
    const response = await httpClient.put<Contract>(`${this.baseUrl}/${id}`, contract)
  ...
  static async getSecrets(id: number): Promise<ContractSecrets> {
    const response = await httpClient.get<ContractSecrets>(`${this.baseUrl}/${id}/secrets`)
