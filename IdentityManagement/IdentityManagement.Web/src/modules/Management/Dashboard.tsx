...
export default function Dashboard() {
  ...
  const filterSessions = (session: ActiveSession) => {
    ...
    return (
      ...
      session.empresaName.toLowerCase().includes(search) ||
      session.sistemaName.toLowerCase().includes(search)
    );
  };
                ...
                >
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {session.userEmail} - {session.empresaName} - {session.sistemaName}
                    </div>
